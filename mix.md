...
### 8. 用框架和不用框架的区别， React和Vue的的区别

`首先说说用框架和不用框架的区别：（以使用框架的角度看)`

a. 使用框架工具写项目，在浏览器中代码依然是原生的HTML CSS JS. 而框架帮开发者做了很多事情。开发者值关注业务逻辑，极大加快了开发速度

例如前端框架解决了`UI与状态同步`的问题，`频繁操作dom`性能低下，`中间步骤`过多，易产生bug且不易维护，而且`心智要求过高`不利于开发效率的一系列的阻碍。

b. 组件化 其中以 react的组件最为彻底，甚至可以到函数级别的原子组件，高度的组件化易于维护，易于组合拓展。

c. 天然分层 JQuery时代 大部分情况下是面条代码  耦合严重 现在框架不管是MVC MVP  还是MVVM模式都能帮助我们进行分层，代码解耦更易于读写。

d. 生态 现在的主流框架都自带生态，不管是数据流管理架构还是UI库都有成熟的解决方案。

`框架的缺点`

a. 代码臃肿，使用者使用框架的时候会引入整个框架，而框架封装了很多功能和组件，使用着必须它的规则使用，而实际开发中很多功能是没有用到的。
...
`react的整体是函数式的思想` 把组件设计成纯组件，状态和逻辑都通过参数传入，所以React都是单向数据流。

`Vue的核心思想是响应式` 也就是基于数据可变的，通过对每一个属性建立的watcher来监听，当属性变化时，响应式更新对于的虚拟dom。

### 9. Session和Cookie的区别

a. `存储的位置不同` cookie的数据信息存在客户端浏览器上，session的数据存放在服务器上。

b. `存储的容量不同` 单个`cookie`只能保存的数据<=4KB, 一个站点最多存储20个Cookie，而对于session来说没有上限，但出于对服务端的性能考虑，
session不要存储过多的东西，并设置session的删除机制。

c. `存储的方式不同` cookie只能保管ASCII字符串，并需要通过编码方式存储为Unicode字符或二进制数据，Session能够存储任何类型的数据，包括不限于String integer list map等

d. `隐私策略不同` cookie是客户端可见的，别有用心的人可以分析存放在本地的cookie并进行cookie欺骗，所以它是不安全的，而session存放在服务器上面，对客户端是透明的，不存在敏感信息泄露的风险

e. `有效期不同` 开发者可以通过设置cookie的属性，达到cookie长期有效的效果。session依赖于名为JSEESION的cookie，而cookie JSESSION的过期时间默认为-1
只需要关闭窗口改session就会失效，因而session不能达到长期的效果。

f. `服务器压力不同` cookie存在在客户端，不占用服务器资源，对于并发用户十分多的网站，cookie是很好的选择，session保存在服务端中，每个用户都会产生一个session。并发用户多的网站，耗费大量的服务器资源。

g. `跨域支持不同` cookie支持跨域访问  session不支持。
